 ...
 *
 *		Note that we simply don't like putting function
 *		definitions in the header file.  The header file is
 *		for the interface.  Actually, we prefer opaque pointer
 *		types in C where you don't even get to see what's
 *		inside, much less touch it.  But, C++ lets us have
 *		value types with access control instead of just always
 *		using pointer types.
 *
 *		Extra functionality:
 ...
 *		The parameter lists are checked for function types.
 */
 ...
 * Function:	Type::operator !=
 *
 * Description:	Well, what do you think it does?  Why can't the
 *		language generate this for us?
 */
 ...
 *
 * Description:	Check if this type is the real type.
 */
 ...
 *		type.  In Simple C, an array is promoted to a pointer, and
 *		an integer is promoted to a real (i.e., double).
 */
...
Type Type::promote() const
{
    ...
    return isInteger() ? Type(DOUBLE) : *this;
}
 ...
 *		type.  Is there an better way than calling assert?
 *		There certainly isn't an easier way.
 */
